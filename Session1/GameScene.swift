...
//    var enemies : [SKSpriteNode] = []
    ...
    func didBeginContact(contact: SKPhysicsContact) {
        ...
        if (maskA | maskB == (PHYSICS_MASK_PLAYER_BULLET | PHYSICS_MASK_ENEMY)) {
            bodyA.node?.removeFromParent()
            bodyB.node?.removeFromParent()
        }
        
        if (maskA | maskB == (PHYSICS_MASK_ENEMY_BULLET | PHYSICS_MASK_PLAYER)) {
            bodyA.node?.removeFromParent()
            bodyB.node?.removeFromParent()
        }
        
        if (maskA | maskB == (PHYSICS_MASK_ENEMY | PHYSICS_MASK_PLAYER)) {
            bodyA.node?.removeFromParent()
            bodyB.node?.removeFromParent()
        }
        
        if (maskA | maskB == (PHYSICS_MASK_POWER_UP | PHYSICS_MASK_PLAYER)) {
            if maskA ==  PHYSICS_MASK_POWER_UP {
                bodyA.node?.removeFromParent()
            }
            else {
                bodyB.node?.removeFromParent()
            }
            playerPlaneController.powerUp()
        }
    }
...
//            self.enemies.append(enemy)
        ...
        let enemySpawnPeriod = SKAction.sequence([enemySpawn, SKAction.waitForDuration(4)])
    ...
    func addPlane() {
        ...
        // 2
        planeView.position = CGPoint(x: self.frame.size.width*0, y: self.frame.size.height/2)
...
//        addChild(planeView)
    ...
    func addPowerUp() {
        ...
        powerUpView.position = CGPoint(x: self.frame.size.width/3, y: self.frame.size.height+30)
